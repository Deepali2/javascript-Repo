...
//helper function to convert a decimal numberf to a binary number
const convertDecimaltoBinary = (num) => {
  let negative = false, binaryString = '';
  while (num > 1) {
    binaryString = String(num % 2).concat(binaryString);
    num = Math.trunc(num / 2);
  }
  return '1'.concat(binaryString)
}

function solution(number) {
  // write your code in JavaScript (Node.js 8.9.4)
  if (number > 2147483647) return;
  const binaryString = convertDecimaltoBinary(number);
  return checkMaxBinaryGap(binaryString);
}
